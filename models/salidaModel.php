...
Class salidaModel extends Model{
	...
	public function salidaProductos($idProducto){		
		$sql=	"
			SELECT 
		`a`.`nIDCOMPRADETALLE` AS `nIDCOMPRADETALLE`
		, `a`.`nIDPRODUCTO` AS `nIDPRODUCTO`
		, `a`.`nCANTIDAD` AS `nCantidadCompra`
		, SUM(IFNULL(`b`.`nCANTIDAD`, 0)) AS `nCantidadVenta`
		, `a`.`nCANTIDAD` - SUM(IFNULL(`b`.`nCANTIDAD`, 0)) AS `nSTOCK` 
		,c.fcostoproduccion
		,c.fganancia
		,SUM(IFNULL(d.ncantidad,0)) AS produccido
			FROM
				(
						`fzbsokgg_tauro_kardex_v2`.`kar_compra_detalle` `a` 
						LEFT JOIN `fzbsokgg_tauro_kardex_v2`.`kar_venta_detalle` `b` 
							ON (
								`a`.`nIDCOMPRADETALLE` = `b`.`nIDCOMPRADETALLE` 
								AND `b`.`nESTADO` = 1
							)
							
						LEFT JOIN kar_compra_taller AS c 
						ON a.nIDCOMPRADETALLE = c.nidcompradetalle
						LEFT JOIN  kar_compra_produccion d
						ON a.nIDCOMPRADETALLE = d.nIDCOMPRADETALLE AND a.nIDPRODUCTO = d.nIDPRODUCTO 
				) 
			WHERE `a`.`bSTOCK` = 1 
				AND `a`.`nESTADO` = 1 
				AND a.nIDPRODUCTO = '$idProducto'
			GROUP BY `a`.`nIDCOMPRADETALLE`
				, `a`.`nIDPRODUCTO`";
		$result = $this->_db->query($sql)or die ('Error en '.$sql);
		return $result;
	}
