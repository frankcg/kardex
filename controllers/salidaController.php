...
class salidaController extends Controller{
	...
	public function salidaProductos(){
		$idProducto = 1;
		$objModel=$this->loadModel('salida');
		$result=$objModel->salidaProductos($idProducto);
		$data = array();
		while($reg=$result->fetch_object()){
			$data[] = array(
				'nIDCOMPRADETALLE'		=> $reg->nIDCOMPRADETALLE,
				'nIDPRODUCTO'			=> $reg->nIDPRODUCTO,
				'nCantidadCompra'		=> $reg->nCantidadCompra,
				'nCantidadVenta'		=> $reg->nCantidadVenta,
				'nSTOCK'				=> $reg->nSTOCK,
				'fcostoproduccion'		=> $reg->fcostoproduccion,
				'fganancia'				=> $reg->fganancia,
				'produccido'			=> $reg->produccido,
			);
		}
		echo json_encode($data);
	}
